...
fn ramanujan(iters: u64) -> f64 {
    ...
    for k in 0..iters {
        ...
        if log_term < -700.0 { break; }
        let term = (1103.0 + 26390.0 * k as f64) * log_term.exp();
        sum += term;
        if term.abs() < 1e-300 { break; }
    }
    1.0 / (factor * sum)
}
...
fn euler_basel(iters: u64) -> f64 {
    let mut s = 0.0_f64;
    for k in 1..=iters { s += 1.0 / (k as f64 * k as f64); }
    (6.0 * s).sqrt()
}
...
impl BenchResult {
    fn new(algo: Algorithm, iters: u64) -> Self {
        ...
        let digits = if error == 0.0 { 15 }
                     else { (-error.log10()).floor().max(0.0) as usize };
        BenchResult { algo, iters, estimate, error, elapsed, digits }
    }

    fn elapsed_us(&self) -> f64 { self.elapsed.as_secs_f64() * 1e6 }
}
...
fn fmt_time(us: f64) -> String {
    if us < 1000.0 { format!("{:.1}μs", us) }
    else           { format!("{:.2}ms", us / 1000.0) }
}

fn print_header() {
    println!();
    println!("  {}{}╔══════════════════════════════════════════════════════════╗{}", BOLD, CYAN, RESET);
    println!("  {}{}║        🔵  THE RACE TO π  —  Algorithms Showdown  🔵   ║{}", BOLD, CYAN, RESET);
    println!("  {}{}╚══════════════════════════════════════════════════════════╝{}", BOLD, CYAN, RESET);
    println!();
    println!("  {}TRUE π{} = {}{}3.14159265358979323846...{}", GREEN, RESET, BOLD, CYAN, RESET);
    println!();
    println!("  {}{}ALGORITHMS:{}", BOLD, GREY, RESET);
    let algos = [Algorithm::Ramanujan, Algorithm::Euler, Algorithm::Wallis, Algorithm::Leibniz];
    for a in algos {
        println!("  {}{}{} {}{}{}",
            a.color(), BOLD, a.symbol(), RESET,
            a.color(), a.label());
        println!("  {}  {}{}",
            " ", DIM, a.formula());
        print!("{}", RESET);
...
fn print_convergence(results: &[Vec<BenchResult>]) {
    println!("  {}{}─── CONVERGENCE TABLE ────────────────────────────────────────{}", BOLD, CYAN, RESET);
    println!();
    println!("  {}{}  {:>6}  {:>4}  {:>4}  {:>4}  {:>4}  {:>4}  {:>4}  {:>4}  {:>4}{}",
        BOLD, WHITE, "ITERS",
        "RAM", "EUR", "WAL", "LBZ",
        "RAM_t", "EUR_t", "WAL_t", "LBZ_t",
        RESET);
...
fn print_visual_race(results: &[Vec<BenchResult>]) {
    println!("  {}{}─── ACCURACY RACE @ 1,000,000 iterations ─────────────────────{}", BOLD, CYAN, RESET);
    println!();
    ...
    for (i, algo) in algos.iter().enumerate() {
        ...
        let c = algo.color();
        println!("  {}{}{} {:>18}{}",
            c, BOLD, algo.symbol(), algo.label(), RESET);
        println!("  {}  {}  {}{:>2} digits correct{}",
            " ".repeat(2),
            bar(r.digits, max_digits, bw, c),
            BOLD, r.digits, RESET);
        println!("  {}  estimate: {}{:.15}{}  ±{:.2e}{}",
            " ".repeat(2), c, r.estimate, RESET, r.error, RESET);
        println!();
    }
}

fn print_verdict(results: &[Vec<BenchResult>]) {
    println!("  {}{}─── VERDICT ───────────────────────────────────────────────────{}", BOLD, CYAN, RESET);
    println!();

    let algos = [Algorithm::Ramanujan, Algorithm::Euler, Algorithm::Wallis, Algorithm::Leibniz];
    let medals = ["🥇", "🥈", "🥉", "  "];

    let mut ranked: Vec<(usize, usize, f64)> = algos.iter().enumerate()
    ...
    for (rank, &(i, digits, us)) in ranked.iter().enumerate() {
        let a = algos[i];
        println!("  {}  {}{}{} {:>18}{}  →  {}{:>2} correct digits{}  in {}{}{}",
            medals[rank],
            a.color(), BOLD, a.symbol(), RESET,
            a.label(),
            BOLD, digits, RESET,
            DIM, fmt_time(us), RESET);
    }

    println!();
    println!("  {}{}◆ Ramanujan adds ~8 decimal digits of precision per iteration.{}", BOLD, RED, RESET);
    println!("  {}  Leibniz needs 10^n terms for n digits — an exponential gap.{}", DIM, RESET);
    println!("  {}  Ramanujan with k=10 already beats Leibniz at k=1,000,000.{}", DIM, RESET);
    println!();
    println!("  {}{}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━{}", BOLD, GREY, RESET);
    println!();
    println!("  {}π showcase (Ramanujan k=10): {}{:.15}{}", DIM, GREEN, ramanujan(10), RESET);
    println!("  {}π showcase (Leibniz 1M):    {}{:.15}{}", DIM, BLUE,  leibniz(1_000_000), RESET);
    println!("  {}π (true f64 constant):      {}{:.15}{}", DIM, CYAN,  PI, RESET);
    println!();
}
...
fn main() {
    ...
    print_header();
    eprintln!("  Computing {} algorithms × {} iteration counts...", algos.len(), iter_sets.len());
    eprintln!();

    let results: Vec<Vec<BenchResult>> = algos.iter().map(|&algo| {
        iter_sets.iter().map(|&n| BenchResult::new(algo, n)).collect()
    }).collect();
